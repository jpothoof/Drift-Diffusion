...
from fipy import Variable, FaceVariable, CellVariable, Grid1D, Grid2D, ExplicitDiffusionTerm, TransientTerm, DiffusionTerm, \
...
ly = 10e-6 # Thickness of system in m
nx = 500  # Number of cells in system in x
ny = 200  # Number of cells in system in y
dy = ly / ny # Thichness of  each cell in m
dx = l / nx  # Length of each cell in m
x = np.linspace(0, l, nx)  # Array to calculate initial values in functions below
y = np.linspace(0, ly, ny)  # Array of y
...
# y03 = np.zeros(nx)
s=(nx, ny)
B01=np.full(s, 0.5e21)
B02=np.full(s, 0.5e21)
B03=np.full(s, 0)

# print(np.shape(B01))

mesh = Grid2D(dx=dx,dy=dy,nx=nx,ny=ny)  # Establish mesh in how many dimensions necessary
mesh_2D = Grid2D(dx=dx,dy=dy,nx=nx,ny=ny)

Pion = CellVariable(mesh=mesh_2D, name='Positive ion Charge Density', value= B01)
Nion = CellVariable(mesh=mesh_2D, name='Negative ion Charge Density', value= B02)
potential = CellVariable(mesh=mesh_2D, name='Potential', value=0.)
#
# print(Pion.value.shape)
# print(Nion.value.shape)
# print(potential.value.shape)

# plt.imshow(Pion.value, vmin=0, vmax = 1e22)
# plt.colorbar()
# plt.show()
...
#
    ...
    # ax5.pcolor(x, y, potential.value)
    # plt.imshow(potential.value)
#
